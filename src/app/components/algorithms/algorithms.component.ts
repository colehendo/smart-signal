...
export class AlgorithmsComponent implements OnInit {
  ...
  appendAlgorithm(timeframe: any) {
    ...
    if (timeframe.value === "month") {
      params = this.selectedAlgorithm.month.params;
    } else if (timeframe.value === "week") {
      params = this.selectedAlgorithm.week.params;
    } else if (timeframe.value === "day") {
      params = this.selectedAlgorithm.day.params;
    } else if (timeframe.value === "four_hour") {
      params = this.selectedAlgorithm.four_hour.params;
    } else if (timeframe.value === "hour") {
      params = this.selectedAlgorithm.hour.params;
    } else if (timeframe.value === "fifteen_minute") {
      params = this.selectedAlgorithm.fifteen_minute.params;
    } else if (timeframe.value === "minute") {
      params = this.selectedAlgorithm.minute.params;
    }
  ...
  removeAlgFromTable(algorithm) {
    this.payload.splice(
      this.payload.indexOf(
        this.payload.find((item) => {
          return (
            item.indicator === algorithm.algorithm &&
            item.timeframe === algorithm.timeframe
          );
        })
      ),
      1
    ...
    this.tableData.splice(
      this.tableData.indexOf(
        this.tableData.find((item) => {
          return (
            item.algorithm === algorithm.algorithm &&
            item.timeframe === algorithm.timeframe
          );
        })
      ),
  ...
  findMaxProfit(time_gap: number) {
    let max_profit_params = new HttpParams().set(
      "time_gap",
      JSON.stringify(time_gap)
    );
  ...
  algoritmHandler(algorithm: any, type: string) {
    isNaN(this.balance) || !this.balance
      ? (this.balance = 100000)
    ...
    type === "algorithm"
      ? this.runAlgorithms(algorithm, all_timeframes)
      : this.runCombinations(algorithm);
  }
  ...
  runAlgorithms(algorithm: any, all_timeframes: any) {
    ...
    this.graph_params = new HttpParams().set(
      "timeframes",
      JSON.stringify(all_timeframes)
    );
    this.alg_params = new HttpParams().set("data", JSON.stringify(algorithm));
    this.updateFlag = false;
    this.apiService
      .getData(this.graph_params)
      .pipe(
        switchMap((data) => {
          let newData = [];
          _.forEach(data[0]["tf_data"], (item) => {
            ...
            if (algorithm.indicator === algorithms.indicator) {
              _.forEach(this.timeframeOptions, (timeframe) => {
                if (algorithm.timeframe === timeframe.value) {
                  combinationString = combinationString.concat(
                    `${algorithms.name} (${timeframe.name}),\n`
                  );
                }
              });
            }
